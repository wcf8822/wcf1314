...
void EC_DE40_SetTds_value(PtrToDOProbe ptd,float Value)
{
	...
	rs485_usart.tx_buf[0]  = EC_DE40_ModbusID;
...
{
	// if(ptd == NULL) return;
	if(ptd->modbus_id == EC_DS46_ModbusID)
	{
		if(len == 0){
		ptd->SN[0]='L';
		ptd->SN[1]='H';
...
void EC_DS46_UpdateTemp2DO(PtrToDOProbe ptd, uint8_t *dat)
{
  ...
  ptd->temperature.value_arr[1] =	dat[16];
	...
	{
		ptd->DOmgl_sum += ptd->pH_Vol.value_f / unit_data;
		ptd->DOpercent_sum += (ptd->DOpercent.value_f);
		ptd->temperature_sum += ptd->temperature.value_f;
		...
		if(++ptd->update_count >= 3)
		{
			...
			if(setting_GetIsOpen_SlideAvg_SAL() && setting_GetSlideAvgTimes_SAL()>=2)//开启并且次数最起码为2次
			{
				if(is_FirstFilter)
				{
					is_FirstFilter = 0;
			...
			if(!DO_GetValueLocked(ptd))
			{
				...
				//盐度
				if(DO_mgl_temp >= 10000)//1000ms ---
				{
					snprintf(ptd->DOpercent_Vol_arr,       7, "%5d",(unsigned int)DO_mgl_temp);			
				}
				else if(DO_mgl_temp >= 1000.0 && DO_mgl_temp < 10000.0)//1000 - 9999 无小数
				{
					snprintf(ptd->DOpercent_Vol_arr,       7, "%5d",(unsigned int)DO_mgl_temp);		
				}
				else if(DO_mgl_temp >= 100.0 && DO_mgl_temp < 1000.0)//100.0 - 999.9 一位小数
				{
					snprintf(ptd->DOpercent_Vol_arr,       7, "%5.1f",DO_mgl_temp);	
				}
				else if(DO_mgl_temp >= 0.0 && DO_mgl_temp < 100.0)//0.00 - 99.99 两位小数
				{
					snprintf(ptd->DOpercent_Vol_arr,       7, "%5.2f",DO_mgl_temp);
				}
			}
			ptd->update_count = 0;
			ptd->DOmgl_sum =0.0;
