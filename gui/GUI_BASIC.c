...
****************************************************************************/
void  GUI_Rectangle(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, TCOLOR color)
{ 
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_RectangleFill(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, TCOLOR color)
{ 
	uint8_t  i;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Square(uint8_t x0, uint8_t y0, uint8_t with, TCOLOR  color)
{   
	if(with==0) return;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, TCOLOR color)
{  
	int32_t   dx;						// 直线x轴差值变量
	int32_t   dy;          			// 直线y轴差值变量
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_LineWith(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t with, TCOLOR color)
{  
	int32_t   dx;						// 直线x轴差值变量
	int32_t   dy;          			// 直线y轴差值变量
	int8_t    dx_sym;					// x轴增长方向，为-1时减值方向，为1时增值方向
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_LineS(uint8_t const *points, uint8_t no, TCOLOR color)
{  
	uint8_t  x0, y0;
	uint8_t  x1, y1;
	uint8_t  i;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Circle(uint8_t x0, uint8_t y0, uint8_t r, TCOLOR color)
{  
	...
	{  
		...
		/* 要判断当前点是否在有效范围内 */
		if( (draw_x0<=GUI_LCM_XMAX)&&(draw_y0>=0) )            GUI_Point(draw_x0, draw_y0, color);   
		if( (draw_x1>=0)&&(draw_y1>=0) )                       GUI_Point(draw_x1, draw_y1, color);
		if( (draw_x2<=GUI_LCM_XMAX)&&(draw_y2<=GUI_LCM_YMAX) ) GUI_Point(draw_x2, draw_y2, color);   
		if( (draw_x3>=0)&&(draw_y3<=GUI_LCM_YMAX) )            GUI_Point(draw_x3, draw_y3, color);
		if( (draw_x4<=GUI_LCM_XMAX)&&(draw_y4>=0) )	           GUI_Point(draw_x4, draw_y4, color);
		if( (draw_x5>=0)&&(draw_y5>=0) )                       GUI_Point(draw_x5, draw_y5, color);
		if( (draw_x6<=GUI_LCM_XMAX)&&(draw_y6<=GUI_LCM_YMAX) ) GUI_Point(draw_x6, draw_y6, color);
		if( (draw_x7>=0)&&(draw_y7<=GUI_LCM_YMAX) )            GUI_Point(draw_x7, draw_y7, color);
	}
}
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_CircleFill(uint8_t x0, uint8_t y0, uint8_t r, TCOLOR color)
{  
	int32_t  draw_x0, draw_y0;			// 刽图点坐标变量
	int32_t  draw_x1, draw_y1;	
	int32_t  draw_x2, draw_y2;	
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Ellipse(uint8_t x0, uint8_t x1, uint8_t y0, uint8_t y1, TCOLOR color)
{  
	int32_t  draw_x0, draw_y0;			// 刽图点坐标变量
   int32_t  draw_x1, draw_y1;
   int32_t  draw_x2, draw_y2;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_EllipseFill(uint8_t x0, uint8_t x1, uint8_t y0, uint8_t y1, TCOLOR color)
{  int32_t  draw_x0, draw_y0;			// 刽图点坐标变量
   int32_t  draw_x1, draw_y1;
   int32_t  draw_x2, draw_y2;
   int32_t  draw_x3, draw_y3;
...
* 说明：若没有找出，则返回最左的x坐标0。
****************************************************************************/
uint8_t  GUI_ReadLeftPoint(uint8_t x0, uint8_t y0, TCOLOR color)
{  uint8_t  i;
   TCOLOR  bakc;
...
****************************************************************************/
uint8_t GUI_ReadRightPoint(uint8_t x0, uint8_t y0, TCOLOR color)
{  
	uint8_t  i;
   TCOLOR  bakc;
...
* 说明：
****************************************************************************/
uint8_t  GUI_CmpPointColor(uint8_t x, uint8_t y, TCOLOR color)
{  
	TCOLOR  bakc;
...
****************************************************************************/
void  GUI_FloodFill(uint8_t x0, uint8_t y0, TCOLOR color)
{  
   ...
   uint8_t  xx, yy;					// 填充临时x，y变量 (当前填充行的中点)
   uint8_t  xx0;						// 当前填充行的左x值变量
   uint8_t  xx1;						// 当前填充行的右y值变量
   uint8_t  i;
   
   uint8_t  x0_bak, y0_bak;
   uint8_t  x1_bak;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Arc4(uint8_t x, uint8_t y, uint8_t r, uint8_t angle, TCOLOR color)
{  
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Arc(uint8_t x, uint8_t y, uint8_t r, uint16_t stangle, uint16_t endangle, TCOLOR color)
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_Pieslice(uint8_t x, uint8_t y, uint8_t r, uint16_t stangle, uint16_t endangle, TCOLOR color)
{  int32_t  draw_x, draw_y;					// 画图坐标变量
   int32_t  op_x, op_y;						// 操作坐标
   int32_t  op_2rr;							// 2*r*r值变量
